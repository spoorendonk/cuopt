...
cuopt_int_t cuOptReadProblem(const char* filename, cuOptOptimizationProblem* problem_ptr)
{
  ...
  try {
    // Dispatches on file extension: ".lp" ⇒ LP parser, otherwise MPS.
    mps_data_model_ptr = std::make_unique<mps_data_model_t<cuopt_int_t, cuopt_float_t>>(
      parse_optimization_file<cuopt_int_t, cuopt_float_t>(filename_str));
  } catch (const std::exception& e) {
    CUOPT_LOG_INFO("Error parsing input file: %s", e.what());
    delete problem_and_stream;
    *problem_ptr        = nullptr;
    std::string err_msg = e.what();
    if (err_msg.find("Error opening MPS file") != std::string::npos ||
        err_msg.find("Error opening LP file") != std::string::npos) {
